...
void SessionStore::reap_expired_sessions() {
    std::vector<std::shared_ptr<SessionState>> expired;
    {
        ...
        reap_expired_sessions_locked(clock_(), expired);
    }
...
ApiResult<SessionSummary> SessionStore::create_session(const SessionCreateRequest& request) {
    if (!enabled()) {
...
ApiResult<SessionSummary> SessionStore::get_session(std::string_view session_id) {
    if (!enabled()) {
        return std::unexpected(disabled_error());
    }
...
ApiResult<void> SessionStore::delete_session(std::string_view session_id) {
    if (!enabled()) {
        return std::unexpected(disabled_error());
    }
...
ApiResult<BeginRequestResult> SessionStore::begin_request(const ChatCompletionRequest& request,
    ...
    const std::int64_t started_at = clock_();
    const ChatMessage user_message = request.messages.front();
    BeginRequestResult result;
    ...
    result.user_message = user_message;
    {
        ...
        result.messages.reserve(session->history.size() + 1);
        result.messages.insert(result.messages.end(), session->history.begin(),
                               session->history.end());
        result.messages.push_back(user_message);
...
void SessionStore::commit_result(std::string_view session_id, std::uint64_t request_id,
...
void SessionStore::release_request(std::string_view session_id, std::uint64_t request_id) {
    std::shared_ptr<SessionState> session;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        auto it = sessions_.find(session_id);
